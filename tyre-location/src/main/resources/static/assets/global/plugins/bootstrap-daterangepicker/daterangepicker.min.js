...
!function (t, e) {
    if ("function" == typeof define && define.amd) define(["moment", "jquery", "exports"], function (a, i, s) {
        ...
        var a = require("moment"), i = "undefined" != typeof window ? window.jQuery : void 0;
        if (!i) try {
            i = require("jquery"), i.fn || (i.fn = {})
        } catch (s) {
    ...
    var s = function (t, e, s) {
        ...
        if ("function" == typeof s && (this.callback = s), this.timePicker || (this.startDate = this.startDate.startOf("day"), this.endDate = this.endDate.endOf("day"), this.container.find(".calendar-time").hide()), this.timePicker && this.autoApply && (this.autoApply = !1), this.autoApply && "object" != typeof e.ranges ? this.container.find(".ranges").hide() : this.autoApply && this.container.find(".applyBtn, .cancelBtn").addClass("hide"), this.singleDatePicker && (this.container.addClass("single"), this.container.find(".calendar.left").addClass("single"), this.container.find(".calendar.left").show(), this.container.find(".calendar.right").hide(), this.container.find(".daterangepicker_input input, .daterangepicker_input i").hide(), this.timePicker || this.container.find(".ranges").hide()), "undefined" != typeof e.ranges || this.singleDatePicker || this.container.addClass("show-calendar"), this.container.addClass("opens" + this.opens), "undefined" != typeof e.ranges && "right" == this.opens) {
            var p = this.container.find(".ranges"), u = p.clone();
            p.remove(), this.container.find(".calendar.left").parent().prepend(u)
        }
    ...
    s.prototype = {
        constructor: s, setStartDate: function (t) {
            ...
            var e = "left" == t ? this.leftCalendar : this.rightCalendar, s = e.month.month(), n = e.month.year(),
                r = e.month.hour(), h = e.month.minute(), l = e.month.second(), o = a([n, s]).daysInMonth(),
                c = a([n, s, 1]), d = a([n, s, o]), m = a(c).subtract(1, "month").month(),
                f = a(c).subtract(1, "month").year(), p = a([f, m]).daysInMonth(), u = c.day(), e = [];
            e.firstDay = c, e.lastDay = d;
            for (var D = 0; 6 > D; D++) e[D] = [];
            var g = p - u + this.locale.firstDay + 1;
            g > p && (g -= 7), u == this.locale.firstDay && (g = p - 6);
            for (var k, v, y = a([f, m, g, 12, h, l]), D = 0, k = 0, v = 0; 42 > D; D++, k++, y = a(y).add(24, "hour")) D > 0 && k % 7 === 0 && (k = 0, v++), e[v][k] = y.clone().hour(r).minute(h).second(l), y.hour(12), this.minDate && e[v][k].format("YYYY-MM-DD") == this.minDate.format("YYYY-MM-DD") && e[v][k].isBefore(this.minDate) && "left" == t && (e[v][k] = this.minDate.clone()), this.maxDate && e[v][k].format("YYYY-MM-DD") == this.maxDate.format("YYYY-MM-DD") && e[v][k].isAfter(this.maxDate) && "right" == t && (e[v][k] = this.maxDate.clone());
            "left" == t ? this.leftCalendar.calendar = e : this.rightCalendar.calendar = e;
            var b = "left" == t ? this.minDate : this.startDate, C = this.maxDate,
                Y = ("left" == t ? this.startDate : this.endDate, '<table class="table-condensed">');
            Y += "<thead>", Y += "<tr>", this.showWeekNumbers && (Y += "<th></th>"), Y += b && !b.isBefore(e.firstDay) || this.linkedCalendars && "left" != t ? "<th></th>" : '<th class="prev available"><i class="fa fa-angle-left"></i></th>';
            ...
            if (this.showDropdowns) {
                ...
                S += "</select>";
                for (var B = '<select class="yearselect">', E = I; x >= E; E++) B += '<option value="' + E + '"' + (E === M ? ' selected="selected"' : "") + ">" + E + "</option>";
                B += "</select>", P = S + B
            }
            if (Y += '<th colspan="5" class="month">' + P + "</th>", Y += C && !C.isAfter(e.lastDay) || this.linkedCalendars && "right" != t && !this.singleDatePicker ? "<th></th>" : '<th class="next available"><i class="fa fa-angle-right"></i></th>', Y += "</tr>", Y += "<tr>", this.showWeekNumbers && (Y += '<th class="week">' + this.locale.weekLabel + "</th>"), i.each(this.locale.daysOfWeek, function (t, e) {
                Y += "<th>" + e + "</th>"
            ...
            for (var v = 0; 6 > v; v++) {
                Y += "<tr>", this.showWeekNumbers && (Y += '<td class="week">' + e[v][0].week() + "</td>");
                for (var k = 0; 7 > k; k++) {
                    var j = [];
                    e[v][k].isSame(new Date, "day") && j.push("today"), e[v][k].isoWeekday() > 5 && j.push("weekend"), e[v][k].month() != e[1][1].month() && j.push("off"), this.minDate && e[v][k].isBefore(this.minDate, "day") && j.push("off", "disabled"), C && e[v][k].isAfter(C, "day") && j.push("off", "disabled"), this.isInvalidDate(e[v][k]) && j.push("off", "disabled"), e[v][k].format("YYYY-MM-DD") == this.startDate.format("YYYY-MM-DD") && j.push("active", "start-date"), null != this.endDate && e[v][k].format("YYYY-MM-DD") == this.endDate.format("YYYY-MM-DD") && j.push("active", "end-date"), null != this.endDate && e[v][k] > this.startDate && e[v][k] < this.endDate && j.push("in-range");
                    for (var H = "", O = !1, D = 0; D < j.length; D++) H += j[D] + " ", "disabled" == j[D] && (O = !0);
                    O || (H += "available"), Y += '<td class="' + H.replace(/^\s+|\s+$/g, "") + '" data-title="r' + v + "c" + k + '">' + e[v][k].date() + "</td>"
                }
                Y += "</tr>"
            }
            ...
            !this.dateLimit || this.maxDate && !this.startDate.clone().add(this.dateLimit).isAfter(this.maxDate) || (s = this.startDate.clone().add(this.dateLimit)), "left" == t ? (a = this.startDate.clone(), i = this.minDate) : "right" == t && (a = this.endDate ? this.endDate.clone() : this.startDate.clone(), i = this.startDate), e = '<select class="hourselect">';
            for (var n = this.timePicker24Hour ? 0 : 1, r = this.timePicker24Hour ? 23 : 12, h = n; r >= h; h++) {
                var l = h;
                this.timePicker24Hour || (l = a.hour() >= 12 ? 12 == h ? 12 : h + 12 : 12 == h ? 0 : h);
                var o = a.clone().hour(l), c = !1;
                i && o.minute(59).isBefore(i) && (c = !0), s && o.minute(0).isAfter(s) && (c = !0), e += l != a.hour() || c ? c ? '<option value="' + h + '" disabled="disabled" class="disabled">' + h + "</option>" : '<option value="' + h + '">' + h + "</option>" : '<option value="' + h + '" selected="selected">' + h + "</option>"
            }
            e += "</select> ", e += ': <select class="minuteselect">';
            for (var h = 0; 60 > h; h += this.timePickerIncrement) {
                var d = 10 > h ? "0" + h : h, o = a.clone().minute(h), c = !1;
                i && o.second(59).isBefore(i) && (c = !0), s && o.second(0).isAfter(s) && (c = !0), e += a.minute() != h || c ? c ? '<option value="' + h + '" disabled="disabled" class="disabled">' + d + "</option>" : '<option value="' + h + '">' + d + "</option>" : '<option value="' + h + '" selected="selected">' + d + "</option>"
            }
            if (e += "</select> ", this.timePickerSeconds) {
                e += ': <select class="secondselect">';
                for (var h = 0; 60 > h; h++) {
                    var d = 10 > h ? "0" + h : h, o = a.clone().second(h), c = !1;
                    i && o.isBefore(i) && (c = !0), s && o.isAfter(s) && (c = !0), e += a.second() != h || c ? c ? '<option value="' + h + '" disabled="disabled" class="disabled">' + d + "</option>" : '<option value="' + h + '">' + d + "</option>" : '<option value="' + h + '" selected="selected">' + d + "</option>"
                }
                e += "</select> "
            }
            if (!this.timePicker24Hour) {
                ...
                var m = "", f = "";
                i && a.clone().hour(12).minute(0).second(0).isBefore(i) && (m = ' disabled="disabled" class="disabled"'), s && a.clone().hour(0).minute(0).second(0).isAfter(s) && (f = ' disabled="disabled" class="disabled"'), e += a.hour() >= 12 ? '<option value="AM"' + m + '>AM</option><option value="PM" selected="selected"' + f + ">PM</option>" : '<option value="AM" selected="selected"' + m + '>AM</option><option value="PM"' + f + ">PM</option>", e += "</select>"
            }
            ...
            if (!this.container.find("input[name=daterangepicker_start]").is(":focus") && !this.container.find("input[name=daterangepicker_end]").is(":focus") && i(t.target).hasClass("available")) {
                var e = i(t.target).attr("data-title"), a = e.substr(1, 1), s = e.substr(3, 1),
                    ...
                    r = n.hasClass("left") ? this.leftCalendar.calendar[a][s] : this.rightCalendar.calendar[a][s];
                ...
                var h = this.leftCalendar, l = this.rightCalendar, o = this.startDate;
                this.endDate || this.container.find(".calendar td").each(function (t, e) {
                    if (!i(e).hasClass("week")) {
                        var a = i(e).attr("data-title"), s = a.substr(1, 1), n = a.substr(3, 1),
                            c = i(e).parents(".calendar"), d = c.hasClass("left") ? h.calendar[s][n] : l.calendar[s][n];
                        d.isAfter(o) && d.isBefore(r) ? i(e).addClass("in-range") : i(e).removeClass("in-range")
                    }
                })
            }
            ...
            if (a) {
                ...
                var o = this.endDate.clone();
                o.hour(s), o.minute(n), o.second(r), this.setEndDate(o)
            }
            ...
            if (this.element.is("input") && this.element.val().length && !(this.element.val().length < this.locale.format.length)) {
                ...
                2 === t.length && (e = a(t[0], this.locale.format), i = a(t[1], this.locale.format)), (this.singleDatePicker || null === e || null === i) && (e = a(this.element.val(), this.locale.format), i = e), e.isValid() && i.isValid() && (this.setStartDate(e), this.setEndDate(i), this.updateView())
            }
        }, keydown: function (t) {
